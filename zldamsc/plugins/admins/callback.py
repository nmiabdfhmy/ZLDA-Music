...
from zldamsc import YouTube, app
from zldamsc.core.call import Zlda
from zldamsc.misc import SUDOERS, db
from zldamsc.utils.database import (is_active_chat,
                                       is_music_playing, is_muted,
                                       is_nonadmin_chat, music_off,
                                       music_on, mute_off, mute_on,
                                       set_loop)
from zldamsc.utils.decorators.language import languageCB
from zldamsc.utils.formatters import seconds_to_min
from zldamsc.utils.inline.play import (panel_markup_1,
                                          panel_markup_2,
                                          panel_markup_3,
                                          stream_markup,
                                          telegram_markup)
from zldamsc.utils.stream.autoclear import auto_clean
from zldamsc.utils.thumbnails import gen_thumb
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await Zlda.pause_stream(chat_id)
        ...
        await Zlda.resume_stream(chat_id)
        await CallbackQuery.message.reply_text(
            _["admin_4"].format(mention)
        )
    elif command == "Stop" or command == "End":
        await CallbackQuery.answer()
        await Zlda.stop_stream(chat_id)
        ...
        await Zlda.mute_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        await Zlda.unmute_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await Zlda.stop_stream(chat_id)
                except:
                    return
        except:
            try:
        ...
        if "live_" in queued:
            ...
            try:
                await Zlda.skip_stream(chat_id, link, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
            ...
            try:
                await Zlda.skip_stream(chat_id, queued, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
